...
for slVal in slopes:
    ...
    plt.scatter(pltLeftk2, pltLeftPhases, color="green", s=sVal * 2, label="left", alpha=0.4)
    plt.scatter(pltRightk2, pltRightPhases, color="red", s=sVal * 2, label="right", alpha=0.4)
    ...
    plt.title("left slope=" + leftSc + ", right slope=" + rightSc + ", $J_{3}/\pi=$" + str(J3Coef))
    ...
    outDir = "./soft10/lin3/J3" + str(J3Coef) + "/slopeStart"+str(slStart)+"slopeEnd"+str(slEnd)+"/"
    Path(outDir).mkdir(parents=True, exist_ok=True)
    # plt.savefig(outDir+"mu"+str(mu)+"J2"+str(J2Coef)+"SoftExpMbcx.png")
    slLeft=round(leftSlope,1)
    slRight=round(rightSlope,1)
    plt.savefig(
        outDir + "leftSlope" + str(slLeft) + "rightSlope" + str(slRight) + "J3" + str(J3Coef) + "soft.png")
    plt.close()
