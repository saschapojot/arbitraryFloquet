...
def U2(b):
    ...
    H2 = 3 * J2 * np.sin(k2) * np.kron(S2, sigma2)+np.kron(np.diag(diagV),sigma0)
...
def U3(b):
    ...
    for n1 in range(0, N1 - 1):
        ...
        S3[n1 + 1, n1] = 1
    S3 *= 3 * J3 / 2
    H3 = np.kron(S3, sigma3)+np.kron(np.diag(diagV),sigma0)
...
def U(b):
    return U3(b).toarray()@U2(b).toarray()@U1Mat.toarray()
...
plt.savefig(outDir+"J2"+str(J2Coef)+"SoftExpMbcx.png")
